...
1.  **Case A (일반 업무):** 중요 거래처가 아니거나 단순 문의인 경우
    * ➔ `sherlock_homes` 내부의 **홈즈(Holmes) 에이전트**가 자체적으로 컨텍스트를 소화하여 처리 및 종결.
2.  **Case B (중요/에스컬레이션 업무):** 중요 거래처이거나 자동 보고서 생성을 요청하는 경우
...
### 왓슨의 핵심 메커니즘
* **감시 및 후킹 (Watch & Hook):** 경찰/조력자 레이어(`police_lestrade_telegram`, `police_anderson_discord` 등)로부터 유저 메시지 및 이벤트를 낚아챔.
* **1차 분류 및 조율 (Validation & Triage):** 인입된 메시지의 발신자(중요 거래처 여부)와 본문(보고서 요청 등의 의도)을 가볍고 빠르게 분석.
* **컨텍스트 스위칭 및 라우팅 (Routing Decision):**
...
이 마크다운을 컨텍스트로 받는 LLM(클로드)은 위 설계를 검증하기 위한 **테스트 하네스(Test Harness) 환경 코드**를 다음 지시사항에 따라 생성하시오.
